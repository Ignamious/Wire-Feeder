...
tireCSDiameter = 0;     // Cross-sectional di	ameter (CS) -- How thick is the tire rubber?
...
treadStyle        = "squares";  // none, cross, o-rings, v-grooves, squares, spheres, cylindersX, 
                                  // 	  cylindersY, cylindersZ, spikes, slots, x, zigX, v
knobSize          = [2,2,2];   // The size of each knob [across wheel, along the perimeter, prodruding]
                                  //   or for v-grooves, [angle, depth, ignored]
radialTreadSets   = 15;           // How many sets of treads to render around the wheel (2 rows per set).
numberOfKnobs     = 1;            // The number of knobs to render per row. 
...
spokeWidth        = 8;          // This is how wide each spoke is.
proportion        = [1,1];  // proportion to rim, proportion of width
curvature         = .9;       // For "spiral", this is how curvey the spokes are. >0, but <=1, where 
                                //     1 is a half circle
...
hubDiameter          = 30;    // The diameter of the hub portion of the wheel
...
innerCircleDiameter  = 35;    // The diameter of the solid inner circle under the hub, or zero for none. 
...
shaftDiameter        = 5.8;          // The diameter of the motor shaft
shaftFlatDiameter    = 5.8
;          // The diameter of the motor shaft at the flat, or shaftDiameter for no flat.

setScrewCount        = 2;          // The number of set screws/nuts to render, spaced evenly around the shaft 
setScrewDiameter     = 3.6;          // The diameter of the set screw. 3 is the default for an M3 screw. 
setScrewTrap         = [6.1, 3.1]; // Size [indiameter, thickness] of set screw nut. The depth is set automatically.
setScrewNutDiameter  = 6;        // The "diameter" of the captive nut, from flat to flat (the "in-diameter")
setScrewNutThickness = 3;        // The thickness of the captive nut
...
$fn = 80;  // Default quality for most circle parts. 
